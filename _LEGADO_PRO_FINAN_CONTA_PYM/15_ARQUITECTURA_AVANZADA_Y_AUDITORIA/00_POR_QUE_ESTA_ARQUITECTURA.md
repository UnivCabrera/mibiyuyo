...
- **Auditoría Obligatoria:** Necesitamos saber _exactamente_ qué pasó, cuándo y quién lo hizo.
...
- **Protección del Núcleo:** Si el SAT cambia su API mañana, solo cambiamos el _Adaptador SAT_. El núcleo de cálculo de impuestos no se toca.
...
En lugar de guardar solo el "estado actual" (ej. "Factura Pagada"), guardamos la _historia_ de todo lo que pasó (`FacturaCreada` -> `FacturaEnviadaSAT` -> `FacturaTimbrada` -> `FacturaPagada`).
