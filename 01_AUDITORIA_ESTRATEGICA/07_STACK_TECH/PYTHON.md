...
Note This is not to be confused with tail call optimization of Python functions, which is currently not implemented in CPython.
...
datetime
Add the strptime() method to the datetime.date and datetime.time classes. (Contributed by Wannes Boeykens in gh-41431.)
...
Add the dis --specialized command-line option to show specialized bytecode. (Contributed by Bénédikt Tran in gh-127413.)
...
platform
Add invalidate_caches(), a function to invalidate cached results in the platform module. (Contributed by Bénédikt Tran in gh-122549.)
...
urllib.parse: Accepting objects with false values (like 0 and []) except empty strings, bytes-like objects and None in parse_qsl() and parse_qs() is now deprecated. (Contributed by Serhiy Storchaka in gh-116897.)
...
See also PEP 739 – build-details.json 1.0 – a static description file for Python build details
