...
export function encryptAPIKey(apiKey: string, userPassword: string): BYOKConfig {
...
export function decryptAPIKey(config: BYOKConfig, userPassword: string): string {
...
| **Visor Documentos** | **PDF.js** (wrapper) | Visualización de facturas y estados de cuenta *in-app* (sin descargas forzadas), permitiendo flujos de revisión rápidos. |
...
const STACK_SOURCES = {
  ...
  drizzle: "https://api.github.com/repos/drizzle-team/drizzle-orm/releases/latest",
...
```json
{
  "mcpServers": {
    ...
    "filesystem": {
      "command": "npx", 
      "args": ["-y", "@modelcontextprotocol/server-filesystem", "/home/red/Documents/PRO_FINAN_CONTA_PYM"]
    }
  }
}
```
